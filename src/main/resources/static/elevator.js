...
var current_floors = new Array();
var current_statuss = new Array();
    ...
    $.ajax({
        ...
        success:function(res){
            for (let i = 0; i < res.length; i++) {
                current_floors[i] = 0;
                current_statuss[i] = 0;
                var floorOnScreen = '#floorOnScreen'+res[i].id;
                //alert(JSON.stringify(res[i]));
                $('#elevatorModel').append('<div class="insideView">\n' +
                    '    <div class="elevator" id="E'+res[i].id+'">\n' +
                    '        <div class="door leftdoor"></div>\n' +
                    '        <div class="door rightdoor"></div>\n' +
                    '        <div class="panel leftpanel">\n' +
                    '        <div style="position: relative;top: 35%">\n' +
                    '            <table id="outsideUpDownButtons" cellspacing="0">\n' +
                    '            <tbody id="tbody'+res[i].id+'">\n' +
                    '            </tbody>\n' +
                    '            </table>\n' +
                    '        </div>\n' +
                    '        </div>\n' +
                    ...
                    '                    <div id="uplight'+res[i].id+'">&#9650;</div>\n' +
                    '                    <div id="downlight'+res[i].id+'">&#9660;</div>\n' +
                    '                </div>\n' +
                    '                <!--显示屏上的数字-->\n' +
                    '                <p class="floorOnScreen" id="floorOnScreen'+res[i].id+'">1</p>\n' +
                    '            </div>\n' +
                    '            <div class="buttons">\n' +
                    '                <div class="button call">&#9990;</div>\n' +
                    '                <div class="button emergency">&#9888;</div>\n' +
                    '                <div class="button open" id="openE'+res[i].id+'">&lt;|&gt;</div>\n' +
                    '                <div class="button close" id="closeE'+res[i].id+'">&gt;|&lt;</div>\n' +
                    '                <div class="dial" id="dialpad'+res[i].id+'">\n' +
                    '\n' +
                    '                </div>\n' +
                    '            </div>\n' +
                    '        </div>\n' +
                    ...
                    '</div>')
                //楼层按钮
                var floorId = '#dialpad'+res[i].id;
                var floorInfo = '<div class="button dial'+-1+'">-1</div>';

                //左侧按钮列
               /* var leftButton = '<tr class="floor" id="floor-1">\n' +
                ...
                floorArray[0] = -1;
                //楼层遍历
                for (let j = 1; j <= res[i].layer_max; j++) {
                    var floorDetail = '<div class="button dial'+j+'">'+j+'</div>';
                    floorInfo+= floorDetail;

                    floorArray[j] = j;

                }
                var leftButtonId = '#tbody'+res[i].id;
                $(floorId).append(floorInfo);
                ...
                for (let j = 0; j < floorArray.length; j++) {
                   /* if(floorArray[j] !== -1){*/
                        var leftButtonSingle = '<tr class="floor" id="floor"'+floorArray[j]+'>\n' +
                            '                            <td class="goup button">▲</td>\n' +
                            '                            <td class="godown button">▼</td>\n' +
                            '                            <td>F'+floorArray[j]+'</td>\n' +
                            '                        </tr>';
                    //}/*else{
                       /* var leftButtonSingle = '<tr class="floor" id="floor"'+floorArray[j]+'>\n' +
                            '                            <td class="goup button">▲</td>\n' +
                   ...
                   leftButton+= leftButtonSingle;
                }

                $(leftButtonId).append(leftButton);
                //楼层信息
                $(floorOnScreen).html(res[i].current_layer);
            }
            console.log(res);
        },
        error:function(){
...
function doIt() {
    $.ajax({
        ...
        success:function(res){
            ...
            for (let i = 0; i < res.length; i++) {
                current_statuss[i] = res[i].door_status;
                if(res[i].current_layer !== current_floors[i]){
                    initLayerLater(res[i].current_layer,res[i].id);
                    current_floors[i] = res[i].current_layer;
                }else{
                    if(current_statuss[i] === 0){
                        close(res[i].id);
                    }else{
                        open(res[i].id);
                    }
...
function init() {
    ...
    for (var i = 0; i < ELE_NUM; i++) {
        ...
        timer[i] = setInterval("main(" + i + ")", 200);
    }
}
$(document).ready(init);
...
function initLayerLater(elevatorNumber,n) {
    var this_class = $(this)[0].className;
    //var parent_id = $(this).parent()[0].id;
...
function openDoor(n) {
    ...
    setTimeout(function () {
        ...
        setTimeout(function () {
            close(n);
            setTimeout(function () {
                timer[n] = setInterval("main(" + n + ")", 100);
            }, 200);
        }, 200);
    }, 100);
}
...
function updateFloorInfo(n) {
    //门的上下移动效果
    var ElevatorMove = (currentFloor[n]) * 588 * 0.05;
