...
public class ElevatorCallable2 implements Callable<ElevatorInfo> {
    ...
    public ElevatorCallable2() {
    }

    public ElevatorCallable2(ElevatorKeys elevatorKeys, ElevatorInfo elevatorInfo, ElevatorMapper elevatorMapper) {
...
/*if(up_down_status == 1){//电梯上行状态
                ...
                if(up_down_status == 2 *//*
*/
/*&& !CollectionUtils.isEmpty(elevatorKeysDown)*//*
*/
/*){//电梯下行状态
    ...
    private  static int runDown(int curr, int go,ElevatorKeysMapper keysMapper,ElevatorKeys elevatorKeys,ElevatorMapper elevatorMapper,ElevatorInfo elevatorInfo) throws InterruptedException {
        int temp = curr;
        List<ElevatorKeys> minRunningTask = keysMapper.selectList(new QueryWrapper<ElevatorKeys>().eq("elevator_id",elevatorKeys.getElevator_id()).in("layer_keys",1,-1));
        ElevatorKeys elevatorKeys1 = minRunningTask.stream().min(Comparator.comparing(n -> n.getLayer())).get();
        if(elevatorKeys1 != null){
            go = elevatorKeys1.getLayer();
        }
